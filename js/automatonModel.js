...
var AUTOMATON_MODEL = (function () {
	...
	// Generate JSON with all the information needed to restore the current automaton
	function save() {
		var i, j, save = {};
		save.stacks = nStacks;
	...
	// Feeded with a JSON generated by the funcion save(), restores the automaton
	...
	// run all the steps until it run into a undefined state or reaches the final state
	function runAll() {
		if (q < 0) q = 0;
	...
	// reset the queue and all the stacks and return to state zero.
	function resetExec() {
			var i;
			q = 0; 
			X = [];
	...
	State.prototype = {
		...
		nextRules: function () {
			...
			// return the indexes of the rules found.
			return rulesToFollow.map(function (rule) { return that.rules.indexOf(rule); });
		},
		// execute rule possible in that state.
		execute: function () {
			var r, rulesToFollow;
	...
	function removeState (index) {
		...
		Q.splice(index, 1); // Q = Q.slice(0,index).concat(Q.slice(index + 1));
		for (i = 0; i < Q.length; i += 1) {
			Q[i].id = i;
		}
	}
